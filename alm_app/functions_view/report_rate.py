...
def get_table_name(fic_mis_date: Union[date, datetime, str]) -> str:
    ...
    table_name = f"Report_rate_sensitive_{date_obj.strftime('%Y%m%d')}"
...
def get_available_report_tables() -> List[Dict[str, Any]]:
    """
    Get list of available Report_rate_sensitive tables from the database
    ...
    with connection.cursor() as cursor:
        ...
        cursor.execute("""
            ...
            WHERE table_schema = 'public' 
              AND table_name LIKE 'report_rate_sensitive_%'
            ...
            try:
                ...
                if len(date_part) == 8 and date_part.isdigit():
                    ...
                    available_tables.append({
                        ...
                        'actual_table_name': f"Report_rate_sensitive_{date_part}",  # Original case
                        'date': date_obj,
    ...
    try:
        with connection.cursor() as cursor:
            ...
            cursor.execute("""
                ...
                ORDER BY table_name
            """, [f"%{table_name.lower().replace('report_rate_sensitive_', '')}%"])
...
@login_required
def get_currencies_api_rate_sensitive(request, fic_mis_date: Union[str, date], process_name: str):
    """
...
@login_required
def rate_sensitive_gap_report_form(request):
    """
    Display form for users to input date and process name for the rate_sensitive gap report
    """
    ...
    if request.method == 'POST':
        ...
        if not fic_mis_date or not process_name:
            ...
            return render(request, 'reports/rate_sensitive/contractual_gap_form.html', {
        ...
        try:
            ...
            if selected_currency == 'all':
                return redirect('alm_app:rate_sensitive_gap_report_cons', 
                              fic_mis_date=date_obj.strftime('%Y-%m-%d'), 
                              process_name=process_name)
            else:
                return redirect('alm_app:rate_sensitive_gap_report_currency_cons', 
                              fic_mis_date=date_obj.strftime('%Y-%m-%d'), 
                              process_name=process_name,
...
@login_required
def rate_sensitive_gap_report_cons(request, fic_mis_date: Union[str, date], process_name: str, currency: str = 'all'):
    """
    ...
    try:
        ...
        if not check_table_exists(table_name):
            ...
            return redirect('alm_app:rate_sensitive_gap_report_form_cons')
        ...
        return render(request, 'reports/rate_sensitive/contractual_gap_report.html', context)
...
@login_required
def rate_sensitive_gap_report_api_cons(request, fic_mis_date: Union[str, date], process_name: str):
    """
