...
def get_table_name(fic_mis_date: Union[date, datetime, str]) -> str:
    ...
    table_name = f"Report_Contractual_Cons_{date_obj.strftime('%Y%m%d')}"
...
def get_available_report_tables() -> List[Dict[str, Any]]:
    """
    Get list of available Report_Contractual_Cons tables from the database
    ...
    with connection.cursor() as cursor:
        ...
        cursor.execute("""
            ...
            WHERE table_schema = 'public' 
              AND table_name LIKE 'report_Contractual_Cons_%'
            ...
            try:
                ...
                if len(date_part) == 8 and date_part.isdigit():
                    ...
                    available_tables.append({
                        ...
                        'actual_table_name': f"Report_Contractual_Cons_{date_part}",  # Original case
                        'date': date_obj,
...
def get_bucket_columns(process_name: str, fic_mis_date: Union[date, datetime, str] = None, base_date: date = None) -> List[Dict[str, Any]]:
    """Get bucket column information for the given process and date with enhanced formatting"""
    qs = TBM.objects.filter(process_name=process_name)
    if fic_mis_date is not None:
        date_obj = _to_date(fic_mis_date)
        qs = qs.filter(fic_mis_date=date_obj)
    buckets = qs.order_by('bucket_number')
    bucket_columns = []
    ...
    try:
        with connection.cursor() as cursor:
            ...
            cursor.execute("""
                ...
                ORDER BY table_name
            """, [f"%{table_name.lower().replace('report_Contractual_Cons_', '')}%"])
...
@login_required
def get_currencies_api_Contractual_Cons(request, fic_mis_date: Union[str, date], process_name: str):
    """
...
@login_required
def Contractual_Cons_gap_report_form(request):
    """
    Display form for users to input date and process name for the Contractual_Cons gap report
    """
    ...
    if request.method == 'POST':
        ...
        if not fic_mis_date or not process_name:
            ...
            return render(request, 'reports/Contractual_Cons/contractual_gap_form.html', {
        ...
        try:
            ...
            # Check if process has bucket configuration for the specific date
            bucket_columns = get_bucket_columns(process_name, date_obj, date_obj)
            if not bucket_columns:
                messages.error(request, 
                    f'No bucket configuration found for process: "{process_name}" and date {date_obj:%Y-%m-%d}. '
                    f'Please ensure TimeBucketMaster is configured for this process and date.')
                return render(request, 'reports/Contractual_Cons/contractual_gap_form.html', {
                    'available_processes': available_processes,
                    'available_tables': available_tables,
                    'fic_mis_date': fic_mis_date,
            ...
            if selected_currency == 'all':
                return redirect('alm_app:Contractual_Cons_gap_report_cons', 
                              fic_mis_date=date_obj.strftime('%Y-%m-%d'), 
                              process_name=process_name)
            else:
                return redirect('alm_app:Contractual_Cons_gap_report_currency_cons', 
                              fic_mis_date=date_obj.strftime('%Y-%m-%d'), 
                              process_name=process_name,
...
@login_required
def Contractual_Cons_gap_report_cons(request, fic_mis_date: Union[str, date], process_name: str, currency: str = 'all'):
    """
    ...
    try:
        ...
        if not check_table_exists(table_name):
            ...
            return redirect('alm_app:Contractual_Cons_gap_report_form_cons')
        
        # Get available currencies for filtering
        available_currencies = get_available_currencies(table_name, process_name)
        
        # Get bucket columns for this process and date
        bucket_columns = get_bucket_columns(process_name, date_obj, date_obj)
        
        if not bucket_columns:
            messages.error(request, f'No bucket columns found for process: {process_name} and date {date_obj:%Y-%m-%d}')
            return redirect('alm_app:Contractual_Cons_gap_report_form_cons')
        ...
        return render(request, 'reports/Contractual_Cons/contractual_gap_report.html', context)
...
@login_required
def Contractual_Cons_gap_report_api_cons(request, fic_mis_date: Union[str, date], process_name: str):
    """
