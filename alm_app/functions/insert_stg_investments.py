...
from .models import Investment, FSI_Loans_Processing  # adjust paths

def insert_stg_investments(fic_mis_date):
    """
    Move rows for fic_mis_date from Stg_Investments -> Fsi_ALM_Processing.
    Upserts on v_account_number. Sets v_loan_type='investments'.
    ...
    existing = {
        x.v_account_number: x
        for x in FSI_Loans_Processing.objects.filter(
    ...
    for src in src_qs:
        data = {
            ...
            "v_interest_payment_type": src.v_interest_type,   # adjust if meaning differs
            ...
            # Target-only fields -> NULL/defaults
    ...
    with transaction.atomic():
        created = FSI_Loans_Processing.objects.bulk_create(to_create, batch_size=1000)
        if to_update:
            update_fields = [f for f in data.keys() if f != "v_account_number"]
            FSI_Loans_Processing.objects.bulk_update(to_update, fields=update_fields, batch_size=1000)
